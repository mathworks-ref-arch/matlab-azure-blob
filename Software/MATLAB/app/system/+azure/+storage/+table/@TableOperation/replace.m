function to = replace(tableEntity, varargin)
...
if numel(tableEntity)==1
    ...
    to = com.microsoft.azure.storage.table.TableOperation.replace(tableEntity.Handle);
else
    ...
    for tCount = 1:numel(tableEntity)
        to.replace(tableEntity(tCount).Handle);
    end
end
