function blob = getAppendBlobReference(obj, varargin)
...
% fully qualified file names. In this case an array of CloudAppendBlobs will be
% returned:
%
%   blobs = container.getAppendBlobReference(myFileCellArray);
%   % with blob names
%   blobs = container.getAppendBlobReference(myFileNameCellArray, myBlobNameCellArray);
%
...
p.FunctionName = 'getAppendBlobReference';
...
if ischar(localFileName)
    ...
    blob = azure.storage.blob.CloudAppendBlob(obj, blobName, localFileName);
end
...
if iscell(localFileName)
    for fCount = 1:numel(localFileName)
        if ~ischar(localFileName{fCount})
            ...
            blob(fCount) = azure.storage.blob.CloudAppendBlob(obj, individualBlobName, localFileName{fCount});
        end
    end
end
